...
import { Pencil, Trash2, ChevronLeft, ChevronRight, Inbox, SearchX, FolderKanban, Download, LucideImage } from "lucide-react";
...
export default function ProjectTable({ onEdit, refreshTrigger, searchQuery = "" }: any) {
  ...
  const fetchData = async () => {
    try { setLoading(true); const res = await ProjectsServices.getDetails(); setDataList(Array.isArray(res) ? res : res?.results || []); }
  ...
  const downloadPDF = () => {
    ...
    autoTable(doc, { head: [["S.N.", "Title", "Description"]], body: paginated.map((item, i) => [(currentPage - 1) * PAGE_SIZE + i + 1, item.title, item.description?.slice(0, 80) + "..."]), startY: 25, styles: { fontSize: 8 }, headStyles: { fillColor: [54, 74, 99] } });
    doc.save("Projects.pdf"); toast.success("PDF Downloaded");
  };
          ...
          <table className="w-full text-left border-separate border-spacing-0">
            <thead className="sticky top-0 z-30 shadow-sm">
              <tr className="bg-[#f5f6fa]">
                ...
                <th className="px-4 py-1.5 text-[11px] font-bold text-[#8094ae] uppercase">Description</th>
                 <th className="px-4 py-1.5 text-[11px] font-bold text-[#8094ae] uppercase">Link</th>
            ...
            <tbody className="divide-y divide-gray-100">
              {loading ? <TableLoadingSkeleton rows={5} cols={7} /> : paginated.length === 0 ? (
                <tr><td colSpan={6} className="text-center py-16"><div className="flex flex-col items-center gap-2">{searchQuery ? <SearchX size={32} className="text-rose-300" /> : <Inbox size={32} className="text-gray-200" />}<span className="text-sm font-bold text-[#364a63]">{searchQuery ? "No results." : "No projects yet."}</span></div></td></tr>
...
<td className="px-4 py-1.5">
  {item.url ? (
    <a
      ...
      className="text-[10px] text-blue-600 hover:text-blue-800 hover:underline font-medium truncate max-w-[150px] block"
      title={item.url}
    >
      {item.url}
    </a>
    ...
    <span className="text-[10px] text-gray-300 italic">No Link</span>
      ...
      <ConfirmModal isOpen={isModalOpen} title="Delete Project?" message={selectedIds.length > 0 ? `Delete ${selectedIds.length} projects?` : "Delete this project?"} onConfirm={handleConfirmDelete} onCancel={() => { setIsModalOpen(false); setDeleteId(null); }} loading={deleteLoading} />
    </div>
  );
}