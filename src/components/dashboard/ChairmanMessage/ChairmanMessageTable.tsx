...
export default function ChairmanMessageTable({
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await ChairmanMessageServices.getDetails();
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      await Promise.all(
        ids.map((id) => ChairmanMessageServices.deleteDetails(id)),
      );
  ...
  const downloadPDF = () => {
    const doc = new jsPDF();
    doc.text("Chairman's Message", 14, 15);
    autoTable(doc, {
      head: [["S.N.", "Title", "Message"]],
